package com.example.busschedule.ui.screen
...
fun BusScheduleApp(
    ...
    val currentScreen = BusScheduleScreens.valueOf(
        currentBackStackEntry.value?.destination?.route?.substringBefore("/") ?: BusScheduleScreens.FullSchedule.name
    )

    val topAppBarTitle = when (currentScreen) {
        BusScheduleScreens.FullSchedule -> fullScheduleTitle
        BusScheduleScreens.RouteSchedule -> currentBackStackEntry.value?.arguments?.getString("busRoute") ?: ""
    }
